import { motion } from "framer-motion";
import { useState, useEffect } from "react";

interface ActiveCallUIProps {
  onDecline: () => void;
}

export function ActiveCallUI({ onDecline }: ActiveCallUIProps) {
  // Live ticking timer logic
  const [time, setTime] = useState(0);
  useEffect(() => {
    const interval = setInterval(() => setTime((t) => t + 1), 1000);
    return () => clearInterval(interval);
  }, []);

  const formatTime = (secs: number) => {
    const m = Math.floor(secs / 60).toString().padStart(2, '0');
    const s = (secs % 60).toString().padStart(2, '0');
    return `${m}:${s}`;
  };

  return (
    <motion.div
      initial={{ opacity: 0, scale: 0.95 }}
      animate={{ opacity: 1, scale: 1 }}
      exit={{ opacity: 0, scale: 0.95 }}
      transition={{ duration: 0.3, ease: "easeOut" }}
      className="flex items-center flex-grow justify-between pl-3.5 pointer-events-auto w-full select-none"
    >
      <div className="flex flex-col items-start min-w-0 pr-2">
        <motion.span layoutId="call-name" className="text-base font-bold tracking-tight text-white whitespace-nowrap overflow-hidden text-ellipsis max-w-[130px]">
          Rudra Gohil
        </motion.span>
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="w-1.5 h-1.5 rounded-full bg-[#00FFA3] animate-pulse" />
          <motion.span layoutId="call-status" className="text-[10px] font-mono text-[#00FFA3] tracking-wider font-semibold">
            {formatTime(time)}
          </motion.span>
        </div>
      </div>
      
      <div className="flex items-center gap-3.5 pr-1">
        {/* Chaotic Active Voice Waveform */}
        <div className="flex gap-1 items-center h-4.5">
          {[...Array(4)].map((_, i) => (
            <motion.div
              key={i}
              animate={{ scaleY: [0.3, Math.random() * 1.3 + 0.4, 0.3] }}
              transition={{ repeat: Infinity, duration: 0.4 + (i * 0.15), ease: "easeInOut" }}
              className="w-[2.5px] h-full bg-[#00FFA3]/80 rounded-full origin-center"
            />
          ))}
        </div>

        {/* Hang Up Button */}
        <button
          onClick={onDecline}
          className="w-9 h-9 rounded-full bg-red-500/20 border border-red-500/40 hover:bg-red-500/35 active:scale-90 transition-all duration-200 cursor-pointer flex items-center justify-center shadow-[0_0_12px_rgba(239,68,68,0.25)] hover:shadow-[0_0_20px_rgba(239,68,68,0.5)] pointer-events-auto"
          title="End Call"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
            className="w-4 h-4 text-white transform rotate-[135deg]"
          >
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
          </svg>
        </button>
      </div>
    </motion.div>
  );
}
